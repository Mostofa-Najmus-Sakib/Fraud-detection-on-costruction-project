function [final_result_cont] = ITD_cont_major(cont_data,pay_affect_cont)
%UNTITLED7 Summary of this function goes here
%%
%major_cell = pay_affect.Major_MinorEffect == 'Major';
%major_cell = {'$Z$61' , '$Z$63'};

% xyz = strcmp(pay_affect_cont.Major_MinorEffect, {'Major'});
major_cell_all = pay_affect_cont(:,:);
...
for y  = 1:length(major_cell)
    disp(major_cell(y))
    cont_data_z_38{y} = cont_data(strcmp(cont_data.Var1,cell2mat(major_cell(y))),:);
    
    for k = 1:length(cont_data_z_38)
        x= cell2table(cont_data_z_38(k));
    end
...
final_result_cont = final_result_major;
